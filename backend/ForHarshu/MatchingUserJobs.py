...
def match_jobs_for_user(user_id: str, max_years_exp: int = 3):
    ...
    setup_logging()
    logger.info(f"Starting job matching process for user: {user_id}, exp filter: <= {max_years_exp} years")
    ...
    try:
        user_profile = UserProfile(
            ...
            resume=Resume(**user_record.resume),
            preferences=user_record.preferences,
            linkedin=user_record.linkedin,
            user_automation_settings=user_record.user_automation_settings
        )
    ...
    if not jobs_to_match:
        logger.info(f"No jobs found with min_exp_required <= {max_years_exp}.")
        return

    logger.info(f"Found {len(jobs_to_match)} jobs to match against.")
        ...
        try:
            ...
            existing_match = (
                ...
                .filter(
                    ...
                    UserJobMatch.job_id == job_record.job_id
                )
                .first()
            )
            if existing_match:
            ...
            logger.error(f"Failed to match job {job_record.job_id} for user {user_id}: {e}")
            continue
    if match_count > 0:
        db.commit()
        logger.info(f"Processed {match_count} matches for user {user_id}.")
    else:
        logger.info("No matches processed.")

    logger.info("Job matching process finished.")


if __name__ == "__main__":
    match_jobs_for_user("harshithavalli6@gmail.com", max_years_exp=3)