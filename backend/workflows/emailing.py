...
from backend.ForHarshu.html_generator import generate_jobs_html, wrap_html_body
...
def get_jobs_for_email(db_session: Session, user_id: str):
    ...
    matches = (
        ...
        .filter(models.UserJobMatch.fit == "Yes")
        .all()
    )
...
def update_job_match_status(db_session: Session, user_id: str, job_ids: list, new_status: str):
    ...
    db_session.query(models.UserJobMatch).filter(
        models.UserJobMatch.user_id == user_id
    ).filter(models.UserJobMatch.job_id.in_(job_ids)).update(
        {"status": new_status}, synchronize_session=False
    )
    db_session.commit()


def run_email_sending(db: Session, user: models.User):
    """Prepares and sends the job recommendations email to a user."""
    logger.info(f"--- Starting Email Sending for user: {user.user_id} ---")
    jobs_to_send = get_jobs_for_email(db, user.user_id)

    if not jobs_to_send:
        logger.info(f"No jobs to send for user {user.user_id}. Exiting.")
        return
    ...
    if send_email(user.user_id, subject, full_html_content):
        job_ids = [job["job_id"] for job in jobs_to_send]
        update_job_match_status(db, user.user_id, job_ids, "EMAIL_SENT")
        logger.info(f"Updated {len(job_ids)} job matches to EMAIL_SENT for user {user.user_id}")

    logger.info(f"--- Email Sending Finished for user: {user.user_id} ---")
